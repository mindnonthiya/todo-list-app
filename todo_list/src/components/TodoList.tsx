...
export default function TodoList() {
  ...
  const [editForm, setEditForm] = useState<TaskFormState>(() => createDefaultFormState());
  ...
  const deleteTodo = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/${activeTodo.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Unable to delete todo");
      setTodos((currentTodos) => currentTodos.filter((todo) => todo.id !== activeTodo.id));
      setDialogMode(null);
      setActiveTodo(null);
    } catch (fetchError) {
      console.error(fetchError);
  ...
  const openEditDialog = useCallback((todo: Todo) => {
    ...
    const dueTime = todo.dueTime ?? "09:00";
    const alarmParts = parseAlarm(todo.alarmDateTime, dueDate, dueTime);
    setActiveTodo(todo);
    setEditForm({
      title: todo.title,
      ...
      alarmDate: alarmParts.date,
      alarmTime: alarmParts.time,
    });
    setDialogMode("edit");
  }, []);
  ...
  }, [activeTodo, editForm, updateTodo]);
  ...
  return (
    <main className="planner-app">
          ...
          <header className="topbar">
            ...
            <div className="header-actions">
              ...
              <div className="user-area">
                ...
                <button type="button" className="avatar-button" aria-label={t("profile")} aria-expanded={profileOpen} onClick={() => setProfileOpen((isOpen) => !isOpen)}><User size={18} /></button>
              </div>
            </div>
            {profileOpen && (
              <UserMenu
      ...
      {dialogMode === "edit" && activeTodo && (
        <Modal title={t("editTask")} onClose={closeDialog}>
          <TaskForm form={editForm} setForm={setEditForm} t={t} onSubmit={saveEditDialog} submitLabel={t("save")} onCancel={closeDialog} compact />
        </Modal>
      )}

      {dialogMode === "delete" && activeTodo && (
...
function UserMenu({ onClose, onProfile, onToggleTheme, themeLabel }: { onClose: () => void; onProfile: () => void; onToggleTheme: () => void; themeLabel: string }) {
  const { t, language, setLanguage } = useLanguage();

  return (
    <div className="popover-layer" role="presentation" onClick={onClose}>
      <div className="profile-menu" role="menu" onClick={(event) => event.stopPropagation()}>
...
function normalizeTodo(todo: Todo): Todo {
  return { ...todo, description: todo.description ?? todo.note ?? "", color: normalizeColor(todo.color), priority: normalizePriority(todo.priority), category: normalizeCategory(todo.category), dueDate: todo.dueDate ?? todo.created_at?.slice(0, 10) ?? todayKey, dueTime: normalizeTime(todo.dueTime), alarmEnabled: Boolean(todo.alarmEnabled ?? todo.alarm) };
}
...
function parseAlarm(value: string | null | undefined, fallbackDate: string, fallbackTime: string) {
  ...
  return { date: toDateInputValue(date), time: `${String(date.getHours()).padStart(2, "0")}:${String(date.getMinutes()).padStart(2, "0")}` };
}
